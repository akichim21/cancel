...
- codex-reviewer が「実行中のまま戻った」「結果なしで終了した」場合、codex 自体は成功していることが多い（サブエージェントはバックグラウンド完了通知で再起動されないため、待機ミスで結果が孤児化する）。**codex を再実行する前に** `ls -lt /tmp/codex-review-output-{短縮名}-*.txt` で候補を探し、`grep -n "^codex" FILE | tail -1` の行以降を最終レスポンスとして回収して精査に進む
  - ⚠️ **回収したファイルが今回の実行のものか必ず確認する。** タイムスタンプが今回の起動時刻より前のものは**別セッション・別 PR の残骸**なので使わない。過去に、固定パスに残っていた別 Issue のレビュー結果を今回の結果として取り込みかけた事故がある
