...
> **GTSS-886 での後続変更**: CSV はさらに末尾へ **配信進捗・初回送信日・支払期日・リマインド送信日** の
> 4列が追加され **22 列**になった。また既存「ステータス」列に新しい表示値「**期限切れ（未回収）**」が
> 出現し得る（請求中かつ支払期日超過の請求。列構成は不変だが値が増える。経理の実取込での受入確認は
> リリース前に実施）。詳細は `docs/product/cancellation-flow.md`「精算用CSVの出力」。
...
- CSV: キャンセル請求管理画面のクライアント側生成CSV（`buildCancellationCsv`）の**末尾に2列を追加**する:
  ...
  - **支払日**: 既存レスポンスの支払い完了日時 `paidAt`（JST 表示）。**未払い行は空**。`paidAt` は Stripe webhook
    （実決済）でのみ設定される。**管理画面の手動「支払済」遷移は廃止**（GTSS-836 / #30）したため、支払日は常に実決済日を表す
    （手動で支払済にして支払日が空のまま精算から漏れる事象を防ぐ）。
...
- 申請一覧・詳細 API（`GET /applications` / `GET /applications/:id`）も **admin 限定（`requireAdmin`）**。代理店コードに加え
  顧客 PII（メール/電話/氏名/生年月日）を含むため、未認証/非管理者には返さない（GTSS-836 / #30 でこの認可を是正）。
  消費者は admin のみ（LP/ユーザーポータルはこれらを呼ばない）。
