...
- **認証画面**（`/verify-email`、`cancel-billing-service-lp/src/components/EmailVerify.jsx`）: URL の `token` を
  ...
  - `verified`（有効期限内）→ 「認証が完了しました」＋「この後の流れ」を表示。申請を **`pending`（審査中）** へ
    ...
    あわせて**有効な運営管理者へ新規申請通知メールを送信**する（未認証段階では送らない。後述「メール送信タイミング」）。
...
>
> **宛先は「有効な」運営管理者に限る**（GTSS-72 / #72）。`role='admin'` かつ `status='active'` の管理者だけへ送り、
> 無効化済み（退職者）は宛先から外す。この通知本文にはサロンの事業者名・代表者名・メールアドレス等の
> 個人情報が含まれるため、無効化済みの宛先を残すことは単なるノイズではなく**個人情報の継続的な送信**にあたる。
> パスワード未設定（招待直後でまだログインしていない）の管理者は正規の運営メンバーなので**含める**。
> 宛先が 1 件も無い場合は警告ログのみで通知をスキップし、呼び出し元の主処理は止めない。
> 管理者アカウントの管理は `docs/product/admin-users.md`。
